...
export default {
	...
	computed: {
		...
		loaddingSt: function () {
			return this.store.lodding
		},
		// 计算购物车清单
		orderAndUserInfo: function () {
			let orderData = []
			Array.isArray(this.store.orderListData) &&
				this.store.orderListData.forEach((n, i) => {
					let userData = {}
	...
	methods: {
		...
		// 设置店铺信息
		setShopInfo(data) {
			this.store.setShopInfo(data)
		},
		// 设置电话
		setShopPhone(data) {
			this.store.setShopPhone(data)
		},
		// 设置店铺状态
		setShopStatus(data) {
			this.store.setShopStatus(data)
		},
		// 设置购物车订单
		initdishListMut(data) {
			this.store.initdishListMut(data)
		},
		// 设置店铺信息
		setStoreInfo(data) {
			this.store.setStoreInfo(data)
		},
		// 设置用户基本信息
		setBaseUserInfo(data) {
			this.store.setBaseUserInfo(data)
		},
		// 设置加载状态
		setLodding(data) {
			this.store.setLodding(data)
		},
		// 设置token
		setToken(data) {
		...
		// 设置配送费
		setDeliveryFee(data) {
			this.store.setDeliveryFee(data)
		},
		loginSync() {
