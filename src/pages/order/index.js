...
export default {
	...
	computed: {
		...
		orderListDataes: function () {
			// 替换为store访问
			return this.store.orderListData
		},
		// 菜品数据
	...
	async onLoad(options) {
		this.initPlatform()
		// 替换为store访问
		this.psersonUrl = this.store.baseUserInfo && this.store.baseUserInfo.avatarUrl
		this.nickName = this.store.baseUserInfo && this.store.baseUserInfo.nickName
		this.gender = this.store.baseUserInfo && this.store.baseUserInfo.gender
		this.remark = this.store.remarkData
		this.init()
		// 存在options说明换地址了
		if (this.store.addressData && this.store.addressData.detail) {
			this.addressBookId = ''
			const newAddress = this.store.addressData
		...
		this.getDateDate()
		this.store.setArrivalTime(this.arrivalTime)
		this.store.setGender(this.gender)
	},
	onReady() {
		uni.getSystemInfo({
			success: (res) => {
	...
	methods: {
		...
		async getEstimatedDeliveryTime() {
			// 替换为store访问
			const result = await getEstimatedDeliveryTime({ shopId: this.store.shopInfo.shopId, customerAddress: this.address });
			this.arrivalTime = window.dayjs(result.data).format('HH:mm')
			this.orderTime = result.data
		},
		// 根据系统派送时间 格式化时间  [16:00,16:30]
		getDateDate() {
			let currentDayjs = window.dayjs(this.orderTime);
			const list = ['立即派送']
		...
		goAddress() {
			// 替换为store方法调用
			this.store.setAddressBackUrl('/pages/order/index')
		...
		payOrderHandle() {
			...
			const params = {
				...
				// 替换为store访问
				shopId: this.store.shopInfo.shopId,
		...
		setTime(val) {
			if (val === '立即派送') {
				this.arrivalTime = window.dayjs(this.orderTime).format('HH:mm')
			} else {
